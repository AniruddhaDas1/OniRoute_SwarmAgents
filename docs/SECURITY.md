...
The v1.0.0-rc1 scope covers the Python runtime, CLI, metadata loading and validation, workflow planning/execution, governance checks, and packaged distribution. Community records and external research links are provenance metadata, not executable dependencies. OniRoute stores no secrets and provides no secrets manager. Automatic AI invocation must be explicitly enabled and governed. Tool/MCP execution is not implemented in runtime v0.6.
