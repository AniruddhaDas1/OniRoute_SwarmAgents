...
class DeterministicExecutor:
    ...
    def execute(self, execution_id: str, plan: ExecutionPlan, optimize: bool | None = None) -> list[ExecutionStep]:
        ...
        for original in sorted(plan.steps, key=lambda step: step.execution_order):
            ...
            if step.skill and self.ai_runner:
                step.result, step.ai_trace = self.ai_runner.run(step.description, set(), optimize=optimize)
