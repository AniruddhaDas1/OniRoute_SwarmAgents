...
description: Design deferred work with idempotency, retry, visibility, and ownership. Use when planning or reviewing background jobs; do not use as provider-specific implementation guidance.
---
# Background Jobs

## Purpose

Design deferred work with idempotency, retry, visibility, and ownership.

## Problem Statement

Without a shared decision model, background jobs is often driven by habit or defaults, producing inconsistent boundaries and hidden trade-offs.

## When to Use

Use when requirements, architecture, reviews, or change plans require decisions about background jobs, before implementation choices become expensive to reverse.
